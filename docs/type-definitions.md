...
- 所有 HTAComponents 函数必须包含 JSDoc 注释
- 使用 `declare namespace HTAComponents` 组织全局声明

### 2. 应用类型定义 (`types/{app-name}.d.ts`)
...
1. **新增 app 时**：同步在 `types/` 中创建 `{app-name}.d.ts`
2. **新增公共组件方法时**：更新 `types/com.d.ts` 或 `types/common.d.ts`
